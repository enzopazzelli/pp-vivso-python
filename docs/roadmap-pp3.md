...
|---|---|---|
| 2026-08-30 | **Las plantillas de viaje estaban invertidas: es "días y noches", no "noches y días"** | El equipo marcó que "2 y 1" / "3 y 2" se usan como un paquete turístico —el primer número son los días totales, el segundo las noches ("3 días 2 noches" es el finde largo clásico)— y el código lo tenía al revés (`VIAJE_CORTO` interpretaba "2 y 1" como 2 noches). Corregido en `rutas/parametros.py`: "2 y 1" ahora es 1 noche (2 días totales, sin ningún día completo en el medio — un viaje corto de ida y vuelta) y "3 y 2" es 2 noches (3 días, 1 día completo en el medio). La fórmula de reparto por día (`presupuesto_por_dia`) no cambió, solo el valor de `noches` que recibe cada plantilla |
| 2026-08-29 | **La dispersión seguía cruzando el límite (círculo isotrópico + eje invertido) y la base del técnico pasa a ser siempre Capital** | El equipo revisó el mapa y marcó dos problemas concretos. (1) Los departamentos se veían como **círculos perfectos**: el tope de la corrección anterior era un radio único (el mínimo de las 4 direcciones) aplicado por igual en las 4, así que Frías y El Charco quedaban apretados también hacia el este —donde no hay límite cerca— y Copo no se abría hacia el sur. Se reemplaza por un tope **direccional** (norte/sur/este/oeste calculados por separado, `_topes_direccionales_km`). Al armarlo apareció un segundo bug, más serio: la conversión final tenía los **ejes cruzados** (el desvío este/oeste se sumaba a la latitud y el norte/sur a la longitud), así que el tope "norte" de Copo nunca actuaba sobre la latitud — quedaba regido por el tope este/oeste, mucho más laxo, y algunos puntos llegaban a pegarse exactos al borde de la caja de seguridad. Corregido; verificado sobre 5.000 filas: 0 puntos fuera de la provincia y 0 pegados al límite (antes del fix del eje, 8 puntos quedaban exactos en el borde). Se sube además `DISPERSION_BASE` (0,02→0,035) y el margen de seguridad (0,5→0,6) ahora que el tope es preciso por dirección y no hace falta ser tan conservador en las direcciones sueltas. (2) **La base de todo técnico es siempre Santiago Capital** (sede del ministerio), no la cabecera de su zona de cobertura como se había asumido — corregido en `rutas/geografia.py:base_tecnico()`. Efecto esperado: las rutas se alargan bastante más (algunos técnicos pasan de ~14h a más de 15h con menos paradas en "2 y 1", porque ahora gastan tiempo real viajando desde el ministerio antes de llegar a su zona) — es el comportamiento correcto, no una regresión |
| 2026-08-29 | **La dispersión geográfica cruzaba el límite provincial; se liga al retraso** | Dos correcciones sobre `[S17]` a partir de revisión del equipo. (1) La dispersión no tenía techo: con departamentos cerca del borde (Copo a 24 km del límite de la caja, Rivadavia a 32 km) y la cola de una normal sin recortar, algunos puntos terminaban en Salta, Catamarca, Santa Fe o Chaco. Se agrega `[S19]`: un radio máximo por departamento (mitad de su distancia real al límite provincial, fuente: extensión geográfica documentada de la provincia) más un recorte final contra la caja completa — verificado en 5.000 filas, 0 puntos fuera. (2) Nuevo supuesto de dominio `[S18]`: las obras de riesgo alto están más lejos de la cabecera ("monte adentro", donde llegan menos visitas y el seguimiento es más débil) — la dispersión ahora se calcula recién después de conocer el `nivel_riesgo` definitivo, no antes. Resultado: riesgo alto promedia 17,2 km de la cabecera contra 5,5 km de riesgo bajo (~3×), y las rutas de `rutas/` se alargaron entre 1,3× y 1,8× en consecuencia — los viajes largos ahora reflejan que las obras urgentes están en el interior de la provincia, no nucleadas en las cabeceras |
| 2026-08-29 | **Generador de rutas para viajes de técnicos (componente extra, plus)** | Un técnico visita obras en varios departamentos muy separados; en vez de un viaje de ida y vuelta por cada una, `rutas/` arma un único viaje de varios días ("2 y 1" o "3 y 2") que encadena visitas pendientes en una ruta eficiente. Dos fases, ambas heurísticas simples y explicables (no un solver): vecino más cercano sobre las **urgentes** (riesgo alto/medio), y en modo mezcla, inserción más barata para sumar pendientes no urgentes que queden de paso sin desviarse mucho — mismo criterio de regla transparente que el modelo de riesgo y `vision/`. No hay red vial real: la distancia se aproxima con línea recta corregida por un factor de ruta `[R2]`. Construido igual que `vision/`: sin consultar antes, con los supuestos `[R#]` como constantes nombradas en `rutas/parametros.py`. Página nueva en el dashboard (`08_rutas.py`), reutilizando `dashboard/components/mapa.py`. Verificado contra los 6 técnicos: con el backlog urgente actual (45 a 274 pendientes de riesgo alto/medio por técnico), un viaje corto se llena solo de urgentes — el modo mezcla se activa recién cuando el backlog urgente baja, verificado simulando viajes consecutivos hasta agotarlo |
| 2026-08-29 | **Corregida la cabecera de Jiménez (Añatuya duplicada)** | Jiménez y General Taboada compartían "Añatuya" como cabecera con coordenadas casi idénticas (~1,5 km de distancia) — Añatuya es la cabecera real de Taboada, no de Jiménez. Verificado en Wikipedia: la cabecera real de Jiménez es **Pozo Hondo** (-27,163, -64,491), a ~145 km de Taboada. Corregido en `LOCALIDADES` (`synthetic/generate.py`); el resto de las 18 localidades de la tabla sigue sin re-verificar contra fuente externa, bajo `[S15]` |
